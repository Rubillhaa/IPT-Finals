...
int main() {
 while (FCGI_Accept() >= 0) {
    ...
    rc = sqlite3_open("/var/www/ipt/rubilla/cgi-bin/db/data.db", &db);
    ...
    const char *sql = id == 0 ? "SELECT * FROM inventory WHERE id > ?" : "SELECT * FROM invetory WHERE id = ?";
    ...
    while ((rc = sqlite3_step(stmt)) == SQLITE_ROW) {
            ...
            json_object_set_new(object, "product", json_string((const char*)sqlite3_column_text(stmt, 1)));
            json_object_set_new(object, "description", json_string((const char*)sqlite3_column_text(stmt, 2)));
            json_object_set_new(object, "quantity", json_string((const char*)sqlite3_column_text(stmt, 3)));
            json_object_set_new(object, "gender", json_string((const char*)sqlite3_column_text(stmt, 4)));
            json_object_set_new(object, "price", json_integer(sqlite3_column_int(stmt, 5)));
