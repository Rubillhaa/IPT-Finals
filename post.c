...
void create_json_array(Report info) {
    ...
    json_object_set_new(obj, "product", json_string(info.product));
    json_object_set_new(obj, "description", json_string(info.description));
    json_object_set_new(obj, "quantity", json_string(info.quantity));
    json_object_set_new(obj, "gender", json_string(info.gender));
    json_object_set_new(obj, "price", json_string(info.price));
...
void loadJSON(Report *info) {  
    ...
    const char* properties[] = { "id", "product", "description", "quantity", "gender", "price" };
    char** infovar[] = {&info->id,&info->product,&info->description,&info->quantity,&info->gender,&info->price};
...
int main(void) {
  ...
  while (FCGI_Accept() >= 0) {
   ...
   const char *sql_statement = "insert into inventory values(?,?,?,?,?,?);";
   ...
   loadJSON(&info);
    ...
    dbres = sqlite3_bind_text(stmt, 2, info.product, -1, SQLITE_TRANSIENT);
    dbres |= sqlite3_bind_text(stmt, 3, info.description, -1, SQLITE_TRANSIENT); 
    dbres |= sqlite3_bind_text(stmt, 4, info.quantity, -1, SQLITE_TRANSIENT);
    dbres |= sqlite3_bind_text(stmt, 5, info.gender, -1, SQLITE_TRANSIENT);
    dbres |= sqlite3_bind_text(stmt, 6, info.price, -1, SQLITE_TRANSIENT);
